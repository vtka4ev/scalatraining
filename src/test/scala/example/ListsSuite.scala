...
class ListsSuite extends FunSuite {
  ...
  test("one plus one is three?") {
    assert(!(Lists.length(1:: List(1)) == 3)) // This assertion fails! Go ahead and fix it.
  }
  ...
  test("negative min of a few numbers") {
    assert(min(List(3, 2, 7,-5,-126,999)) === -126)
  }

  test("min throws an NoSuchElementException if its argument is empty list") {
    intercept[NoSuchElementException] {
      min(List())
    }
  }

  test("length of the empty list equals 0") {
